...
export default {
  fetchUsers: params => {
    return dispatch => {
      return dispatch(
        TurboClient.getRequest("user", params, constants.USERS_RECEIVED)
      );
    };
  },
  ...
  addUser: params => {
    return dispatch => {
      return dispatch(
        TurboClient.postRequest("user", params, constants.USER_CREATED)
      );
    };
  },
  ...
  loginUser: credentials => {
    return dispatch => {
      return dispatch(
        TurboClient.login(credentials, constants.CURRENT_USER_RECEIVED)
      );
    };
  },
  ...
  updateCurrentUser: (currentUser, params) => {
    return dispatch => {
      return dispatch(
        TurboClient.updateCurrentUser(
          currentUser,
          params,
          constants.CURRENT_USER_UPDATED
        )
      );
    };
  }
